...
if ( ! class_exists( 'WpssoFaqIntegAdminPost' ) ) {

	class WpssoFaqIntegAdminPost {
		...
		public function add_wp_callbacks() {
			 ...
			 */
			add_filter( 'manage_' . WPSSOFAQ_QUESTION_POST_TYPE . '_posts_columns', array( $this, 'add_post_column_headings' ), 10, 1 );

			/*
			 * See https://codex.wordpress.org/Plugin_API/Action_Reference/manage_$post_type_posts_custom_column.
			 */
			add_action( 'manage_' . WPSSOFAQ_QUESTION_POST_TYPE . '_posts_custom_column', array( $this, 'show_column_content' ), 10, 2 );
		...
		public function maybe_show_edit_form_after_title( $post_obj ) {
			...
			if ( WPSSOFAQ_QUESTION_POST_TYPE === $post_type ) {
				...
				} else echo __( 'Edit Answer', 'wpsso-faq' );
